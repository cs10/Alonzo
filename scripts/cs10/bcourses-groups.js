...
// Try to be lenient
var matchGroupForSID = /(.+?)\s*(group|grp|grop)\s*(for|fr|fo)?\s*(\d+)/i;

module.exports = function(robot) {

    // Tries to match '<assignment-name> group for <sid>'
    // TODO: HANLDE PAGINATION AND RATE LIMITING then this can be activated
    // robot.respond(matchGroupForSID, {id: 'cs10.bcourses-groups.group-for-sid'}, function(msg) {
...
function getGroupForSID(assgn, sid, cb) {
    ...
    if (!catID) {
        ...
        cb({
            msg: `Could not find a group with that name. Possible groups are:\n ${groups}`
        }, null);
        return;
    }
    ...
    cs10.get(`/group_categories/${catID}/groups`, {
        per_page: 100
    }, function(error, resp, groups) {

        if (error) {
            cb({
                msg: `Could not find a matching group for student: ${sid}`
            }, null);
            return;
        }
